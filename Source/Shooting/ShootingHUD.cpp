...
{
    Super::DrawHUD();
    const FString MapName = GetWorld()->GetMapName().Mid(GetWorld()->StreamingLevelsPrefix.Len());
    ...
    float ScoreTextY = 10.0f;
    {
        ...
        const FString PlayerName = Cast<UShootingGameInstance>(GetGameInstance())->GetPlayerName();
        const FText PlayerText = FText::FromString(FString("Current Player - ") + PlayerName);
        DrawText(PlayerText.ToString(), FColor::Orange, 10.0f, ScoreTextY, Font, 3);
        ScoreTextY += 40.0f;
        const FText WeaponText = FText::FromString(FString("Current Weapon: ") + PlayerState->GetWeaponName());
        DrawText(WeaponText.ToString(), FColor::Orange, 10.0f, ScoreTextY, Font, 3);
        ScoreTextY += 40.0f;
    }
    for (AShootingPlayerState* PlayerState : ShootingPlayerArray)
    {
        FString PlayerName = PlayerState->GetPlayerName();
        const uint32 CurrentScore = PlayerState->GetShootingScore();
    ...
    {
        const int32 Time = Cast<AShootingCharacter>(GetOwningPlayerController()->GetPawn())->GetTimeRemaining();
        if (Time < 0)
        {
        ...
        else
        {
            ...
            const FText TimeText = FText::Format(FTextFormat::FromString("{0}:{1}"), FText::AsNumber(Minute, &Options), FText::AsNumber(Second, &Options));
